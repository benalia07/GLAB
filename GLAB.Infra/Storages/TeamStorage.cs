...
namespace GLAB.Infra.Storages
{
    internal class TeamStorage : ITeamStorage
    {
        ...
        public async Task<Team> SelectTeamById(string TeamId)
        {
            Team team = new Team();
            await using var connection = new SqlConnection(connectionString);
            SqlCommand cmd = new("select * from dbo.Team where TeamId = @aTeamId", connection);
        ...
        {
            await using var connection = new SqlConnection(connectionString);
            SqlCommand cmd = new("Insert dbo.Team(TeamId，Status,LaboratoryId) " +
        ...
        {
            await using var connection = new SqlConnection(connectionString);
            SqlCommand cmd = new("DELETE FROM dbo.Team WHERE TeamId = @TeamId", connection);
        ...
        {
            await using var connection = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand("UPDATE dbo.Team SET LaboratoryId = @aLaboratoryId, Status = @Status WHERE TeamId = @TeamId", connection);
